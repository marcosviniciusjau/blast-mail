<div class="grid grid-cols-2 gap-4">
    ...
    <div>
        ...
        <x-select name="email_list_id" id="email_list_id">
            <option value="" @if(blank(old('email_list_id', $data['email_list_id']))) selected @endif>{{ __('Select a email') }}</option>
    ...
    <div>
        ...
        <x-select name="template_id" id="template_id">
            <option value="" @if(blank(old('template_id', $data['template_id']))) selected @endif>{{ __('Select a template') }}</option>
    ...
    <div>
        <x-input.checkbox id="track_click" name="track_click" value="1" :isCheckedWhen="old('track_click', $data['track_click'])" :label="__('Track Click')" autofocus />
        <x-input.error :messages="$errors->get('track_click')" class="mt-2" />
    </div>

    <div>
        <x-input.checkbox id="track_open" name="track_open" value="1" :isCheckedWhen="old('track_open', $data['track_open'])" :label="__('Track Open')" autofocus />
